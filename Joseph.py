...
@st.cache_data
def load_data():
    health = pd.read_csv("data/merged_health_data.csv")
    no2 = pd.read_csv("data/nitrogen_dioxide.csv")
    o3 = pd.read_csv("data/ozone.csv")
...
def train_models(X, y_asthma, y_resp):
    ...
    asthma_mae = -cross_val_score(asthma_model, X, y_asthma, scoring='neg_mean_absolute_error').mean()
    resp_mae = -cross_val_score(resp_model, X, y_resp, scoring='neg_mean_absolute_error').mean()
...
with st.expander("📊 Model Performance Report (SO₂ Basis)"):
    ...
    with col1:
        st.metric("Asthma Model", f"R²: {asthma_r2:.2f}", f"MAE: {asthma_mae:.1f} cases/10k")
    with col2:
        st.metric("Respiratory Model", f"R²: {resp_r2:.2f}", f"MAE: {resp_mae:.1f} cases/10k")
...
if pollutant_type != "SO2":
    st.warning(f"⚠️ Comparative Analysis: Showing {pollutant_type} levels as if they had the same concentration-health relationship as SO₂.")
    
    if value < so2_range[0] or value > so2_range[1]:
        st.error(f"🚨 Input value ({value:.1f} {unit}) outside SO₂ training range ({so2_range[0]:.1f}-{so2_range[1]:.1f} tons/year).")
...
if st.button("Run Health Impact Simulation"):
    ...
    ax.scatter(X, y_asthma, alpha=0.4, c='#1f77b4', s=60, label='Actual Asthma Cases')
    ax.scatter(X, y_resp, alpha=0.4, c='#2ca02c', s=60, label='Actual Respiratory Cases')
    
    pred_color = '#d62728' if pollutant_type != "SO2" else '#9467bd'
    ax.scatter(value, asthma_pred, s=400, marker="*", edgecolor='black', linewidth=1, c=pred_color, label=f'Asthma Prediction ({pollutant_type})')
    ax.scatter(value, resp_pred, s=400, marker="P", edgecolor='black', linewidth=1, c=pred_color, label=f'Respiratory Prediction ({pollutant_type})')
    
    ax.axvspan(so2_range[0], so2_range[1], alpha=0.1, color='gray', label='SO₂ Training Range')
    ax.axhline(y_asthma.median(), color='#1f77b4', linestyle=':', linewidth=1.5, label='Asthma Median')
    ax.axhline(y_resp.median(), color='#2ca02c', linestyle=':', linewidth=1.5, label='Respiratory Median')
    
    ax.annotate(f'{asthma_pred:.1f}', (value, asthma_pred), xytext=(15, -15), textcoords='offset points', arrowprops=dict(arrowstyle="->", color='black'), fontsize=10, weight='bold')
    ax.annotate(f'{resp_pred:.1f}', (value, resp_pred), xytext=(15, 15), textcoords='offset points', arrowprops=dict(arrowstyle="->", color='black'), fontsize=10, weight='bold')
    ...
    ax.set_title(f"Health Impact Projection: {pollutant_type} vs. SO₂ Model", fontsize=14, pad=20)
...
with st.expander("🔍 Explore Source Datasets"):
    ...
    with tab2:
        ...
        styled_no2 = no2_df.style.format({'data_value': "{:.1f} ppb"}).background_gradient(subset=['data_value'], cmap='Blues')
        st.dataframe(styled_no2, height=300)
    
    with tab3:
        st.write("**Ozone Measurements**")
        styled_o3 = o3_df.style.format({'data_value': "{:.1f} ppb"}).background_gradient(subset=['data_value'], cmap='Greens')
...
---
**🔬 Scientific Context**  
- Model trained on SO₂ emissions ({:.1f}-{:.1f} tons/year)  
- Cross-pollutant assumption: linear concentration-response  
- Caution: toxicity profiles, synergistic effects not accounted for

*Research purpose only - consult epidemiology studies for causal inference.*
""".format(so2_range[0], so2_range[1]))